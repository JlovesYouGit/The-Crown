...
class ConsciousnessLoop:
    ...
    class _SafetyController:
        ...
        def risk_score(
            ...
            *,
            ...
            # If in holdoff, amplify score to encourage reduced actions
            if is_in_holdoff:
        ...
        def enforce_policy(self, *, risk: Dict[str, Any]) -> Dict[str, Any]:
            ...
            # Enter hold-off
            self._holdoff_until = max(self._holdoff_until, risk.get("iteration_index", 0) + self.holdoff_iterations)
            return {
                "action": "holdoff_higher_accel_zone",
                # Higher acceleration zone in practice = reduced boost + reduced impact
    ...
    def iterate(self) -> Dict[str, Any]:
        ...
        # Safety verification / "heat of death" detection
        ...
        policy = self._safety.enforce_policy(risk=risk)

        if policy["action"] != "proceed":
            # Avoid direct impact: skip modification or apply reduced boost
            safety_event = {
                "iteration": self.iteration_count,
            ...
            # Regulated aggression / higher acceleration zone (safer): reduce density_boost, then proceed
