...
# Molecular Physics Integration
from engine.core.molecular_physics import MolecularPhysicsEngine
...
class NodeGraph:
    ...
    def __init__(self, backup_dir: Path = Path("data/backups"), obj_file_path: Path = None):
        ...
        # Molecular Physics Integration
        self.molecular_physics = MolecularPhysicsEngine(obj_file_path=obj_file_path, num_molecules=100)
        # Register knowledge callback to update graph with molecular state
        self.molecular_physics.register_knowledge_callback(self._on_molecular_state_update)
    ...
    def bootstrap(
        ...
        self.molecular_physics.start()
        logger.info("Molecular physics engine started")
    ...
    def query(self, text: str, top_k: int = 3) -> dict:
        ...
        enriched_result = self.enricher.enrich(text, raw_result)
        
        # Molecular Physics Integration
        if self.molecular_physics.running:
            molecular_data = self.molecular_physics.query_molecular_state(text)
            enriched_result['molecular_physics'] = molecular_data
            # Combine molecular resonance with graph resonance
            molecular_resonance = molecular_data.get('data', {}).get('collective_resonance', 0)
            enriched_result['combined_resonance'] = round(
                enriched_result.get('resonance_score', 0) * 0.7 + molecular_resonance * 0.3, 10
            )
    ...
    def index_text(self, text: str, metadata: dict | None = None) -> list[str]:
        ...
        # Molecular Physics Integration
        if self.molecular_physics.running:
            molecular_hash = self.molecular_physics.index_molecular_knowledge(text, metadata)
            hashes_written.append(molecular_hash)
    ...
    def stats(self) -> dict:
        ...
        # Add molecular physics stats
        base_stats["molecular_physics"] = self.molecular_physics_status()
    ...
    def _on_molecular_state_update(self, state: dict) -> None:
        """Callback for molecular physics state updates."""
        # Store molecular state in semantic map for knowledge integration
        molecular_key = f"molecular_state_{state['step_count']}"
        self.semantic_map.index(molecular_key, {
            'type': 'molecular_physics',
            'resonance': state['collective_resonance'],
            'fluidity': state['fluidity'],
            'cohesion': state['cohesion'],
            'timestamp': time.time()
        })
        logger.debug(f"Molecular state updated: resonance={state['collective_resonance']:.4f}")

    def start_molecular_physics(self) -> None:
        """Start molecular physics engine."""
        if not self.molecular_physics.running:
            self.molecular_physics.start()
            logger.info("Molecular physics engine started")

    def stop_molecular_physics(self) -> None:
        """Stop molecular physics engine."""
        if self.molecular_physics.running:
            self.molecular_physics.stop()
            logger.info("Molecular physics engine stopped")

    def molecular_physics_status(self) -> dict:
        """Get molecular physics engine status."""
        return self.molecular_physics.status()
